        ...
        <section className="contact-section">
          <div className="contact-grid">
            <div className="contact-info">
              <p className="section-eyebrow" style={{ justifyContent: "flex-start" }}>{t("footer.visitUs")}</p>
              <h2 className="contact-title">{t("footer.findUs")}</h2>
  ...
  return (
    <ThemeProvider>
      <AuthProvider>
        <CartProvider>
          {trackingOpen ? (
            <OrderTrackingPage onClose={() => { setTrackingOpen(false); setTrackingOrderId(null) }} orderId={trackingOrderId} />
          ) : (
            <ShopPage onOrdersClick={() => setTrackingOpen(true)} onAccountClick={() => setAccountOpen(true)} />
          )}
          <AccountPage isOpen={accountOpen} onClose={() => setAccountOpen(false)} onViewOrder={handleViewOrder} />
        </CartProvider>
      </AuthProvider>
    </ThemeProvider>
  )
